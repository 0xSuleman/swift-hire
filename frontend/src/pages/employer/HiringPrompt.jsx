    ...
    try {
      ...
      if (d.candidatesFound === 0) {
        ...
        setParsed({
          ...
          parsedLocation:   d.parsedLocation,
          parsedShift:      d.parsedShift,
          parsedExperience: d.parsedExperience,
        })
      }
    } catch (err) {
      ...
      <div style={{ maxWidth: 680 }}>
        {parsed !== null ? (
          <div className="app-card">
            <p style={{ fontWeight: 700, color: '#E8EAF0', fontSize: '1rem', marginBottom: 16 }}>
              What we detected
            </p>

            {/* Skills */}
            <p style={{ color: '#6B7280', fontSize: '0.78rem', marginBottom: 6 }}>Skills</p>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 14 }}>
              {Array.from(parsed.parsedSkills ?? []).map(s => (
                <span key={s} style={{ background: '#2EE5B015', border: '1px solid #2EE5B030', color: '#2EE5B0', borderRadius: 6, padding: '2px 8px', fontSize: '0.78rem' }}>{s}</span>
              ))}
              {(!parsed.parsedSkills || parsed.parsedSkills.length === 0) && (
                <span style={{ color: '#4B5563', fontSize: '0.78rem' }}>None detected</span>
              )}
            </div>

            {/* Location, Shift, Experience row */}
            <div style={{ display: 'flex', gap: 20, marginBottom: 16, flexWrap: 'wrap' }}>
              <div>
                <p style={{ color: '#6B7280', fontSize: '0.72rem', marginBottom: 2 }}>Location</p>
                <p style={{ color: '#E8EAF0', fontSize: '0.85rem', fontWeight: 500 }}>{parsed.parsedLocation ?? 'Not specified'}</p>
              </div>
              <div>
                <p style={{ color: '#6B7280', fontSize: '0.72rem', marginBottom: 2 }}>Shift</p>
                <p style={{ color: '#E8EAF0', fontSize: '0.85rem', fontWeight: 500 }}>{parsed.parsedShift ?? 'Not specified'}</p>
              </div>
              <div>
                <p style={{ color: '#6B7280', fontSize: '0.72rem', marginBottom: 2 }}>Experience</p>
                <p style={{ color: '#E8EAF0', fontSize: '0.85rem', fontWeight: 500 }}>{parsed.parsedExperience != null ? parsed.parsedExperience + ' years' : 'Not specified'}</p>
              </div>
              <div>
                <p style={{ color: '#6B7280', fontSize: '0.72rem', marginBottom: 2 }}>Candidates Found</p>
                <p style={{ color: '#2EE5B0', fontSize: '0.85rem', fontWeight: 700 }}>{parsed.candidatesFound}</p>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button
                ...
                onMouseEnter={e => { e.currentTarget.style.filter = 'brightness(1.08)'; e.currentTarget.style.boxShadow = '0 0 28px rgba(46,229,176,0.35)' }}
                onMouseLeave={e => { e.currentTarget.style.filter = 'none'; e.currentTarget.style.boxShadow = '0 0 16px rgba(46,229,176,0.2)' }}
                onClick={() => navigate('/employer/candidates/' + parsed.jobPostingId)}
              >
                View {parsed.candidatesFound} Candidate{parsed.candidatesFound !== 1 ? 's' : ''} →
              </button>
              <button
                style={{
                  padding: '9px 22px', borderRadius: 9999,
                ...
                onMouseEnter={e => { e.currentTarget.style.borderColor = '#6B7280'; e.currentTarget.style.color = '#E8EAF0' }}
