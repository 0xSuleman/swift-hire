...
export default function RateCandidate() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      setSuccess(true)
      setTimeout(() => navigate('/employer/interviews'), 1800)
    } catch (err) {
    ...
    <AppLayout>
      <div style={{ maxWidth: 480 }}>
        <div className="page-header">
          <h1 className="page-title">Rate Candidate</h1>
          <p className="page-subtitle">Your feedback helps improve future matching.</p>
        </div>

        {slot && (
          <div className="app-card" style={{ marginBottom: 20, display: 'flex', flexDirection: 'column', gap: 6 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <User size={14} style={{ color: '#2EE5B0' }} />
              <span style={{ fontSize: '0.9rem', fontWeight: 600, color: '#E8EAF0' }}>{slot.candidate}</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            {slot.candidateEmail && (
              <span style={{ fontSize: '0.75rem', color: '#4B5563' }}>{slot.candidateEmail}</span>
            )}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit}>
          <div className="app-card" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div>
              ...
              <textarea
                rows={4}
                placeholder="e.g. Strong technical skills, communicated well during the interview..."
                value={comment}
